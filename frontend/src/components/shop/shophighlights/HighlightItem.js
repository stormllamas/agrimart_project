...
const HighlightItem = ({ highlightitem }) => {
  return (
    <li className="best-seller-product col">
      <Link to={`/shop/product?p=${highlightitem.id}&b=${product.seller.name_to_url}`}>
      ...
      <div className="best-seller-product-info">
        <Link to={`/shop/product?p=${highlightitem.id}&b=${product.seller.name_to_url}`} className="best-seller-product-name"><h5>{ highlightitem.name }</h5></Link>
